...
function SendMassage() {
	var firebaseRef = firebase.database().ref("ChatSystem");
	var groupname = localStorage.getItem("groupEntry");
	var chatRef = firebase.database().ref("ChatSystem/"+groupname+"_chatroom");
	var sender = localStorage.getItem("Username");
	var chat_message = document.getElementById('TBox').innerHTML;
...
function ShowRoomMessage() {
	...
	chatRef.limitToLast(12).on("child_added", function(data){
		messageData = data.val();
		if(messageData.name == sender) {
			$(chatRoom).append("<div id='myMsg'><span id='myMsgText'>" + messageData.message + "</span>" + " <span id='myMsgName'> :" + messageData.name + " </span></div><br>");
			console.log(messageData.name.length);
		} else {
			$(chatRoom).append("<div id='otherMsg'><span id='otherMsgName'>" + messageData.name + ":</span> " + "<span id='otherMsgText'>" + messageData.message + "</span></div><br>");
		}
	});
}
